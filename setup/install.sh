...
gcloud builds submit ../event_handler --tag=gcr.io/${FOURKEYS_PROJECT}/event-handler${IMAGE_TAG} --project=${PARENT_PROJECT} > event_handler.containerbuild.log

if [[ ! -z "$GIT_SYSTEM" ]]; then
    gcloud builds submit ../bq-workers/${GIT_SYSTEM}-parser --tag=gcr.io/${FOURKEYS_PROJECT}/${GIT_SYSTEM}-parser${IMAGE_TAG} --project=${PARENT_PROJECT} > ${GIT_SYSTEM}-parser.containerbuild.log
fi

if [[ ! -z "$CICD_SYSTEM" && "$CICD_SYSTEM" != "$GIT_SYSTEM" ]]; then
    gcloud builds submit ../bq-workers/${CICD_SYSTEM}-parser --tag=gcr.io/${FOURKEYS_PROJECT}/${CICD_SYSTEM}-parser${IMAGE_TAG} --project=${PARENT_PROJECT} > ${CICD_SYSTEM}-parser.containerbuild.log
fi

# Dashboard image
gcloud builds submit ../dashboard --tag=gcr.io/${FOURKEYS_PROJECT}/fourkeys-grafana-dashboard${IMAGE_TAG} --project=${PARENT_PROJECT} > fourkeys-grafana-dashboard.containerbuild.log
